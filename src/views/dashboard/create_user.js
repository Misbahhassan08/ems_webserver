...
const CreateUser = () => {
  ...
  useEffect(() => {
    if (isEditMode) {
      setFormData({
        ...
        created_by_id: receivedUser.created_by?.user_id || '',
      });
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        const res = await axios.get(urls.admins);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const createdById = formData.created_by_id || getUserIdFromLocalStorage();
      const userPayload = {
        ...formData,
    ...
    <Box sx={{ maxWidth: 800, mx: 'auto', p: 4 }}>
      ...
      <Box sx={{ display: 'flex', gap: 3 }}>
        <Box sx={{ flex: 1 }}>
          ...
          <TextField name="adress" label="Address" value={formData.adress} onChange={handleChange} fullWidth sx={{ mb: 2 }} error={!!errors.adress} helperText={errors.adress} />
          <TextField name="zip_code" label="Zip code" value={formData.zip_code} onChange={handleChange} fullWidth sx={{ mb: 2 }} error={!!errors.zip_code} helperText={errors.zip_code} />

          {!isEditMode && (
            <TextField name="password" label="Password *" value={formData.password} onChange={handleChange} fullWidth type="password" sx={{ mb: 2 }} error={!!errors.password} helperText={errors.password} />
          )}

          {role === 'superadmin' && isEditMode && (
            <Select
              name="created_by_id"
              value={formData.created_by_id || ''}
              onChange={(e) => setFormData({ ...formData, created_by_id: e.target.value })}
              fullWidth
              displayEmpty
              sx={{ mb: 2 }}
            >
        ...
        <Box sx={{ flex: 1 }}>
          ...
          {errors.image && <Typography sx={{ color: 'red', mt: 1 }} variant="body2">{errors.image}</Typography>}
          ...
          {role === 'admin' && (
            <Select value={selectedGateway} onChange={(e) => { setSelectedGateway(e.target.value); handleSelectGateway(e.target.value); }} fullWidth displayEmpty sx={{ mt: 2 }}>
              <MenuItem value="" disabled>Select an Unassigned Gateway</MenuItem>
              {unassignedGateway.map((gw) => (
                <MenuItem key={gw.G_id} value={gw.G_id}>{gw.gateway_name}</MenuItem>
              ))}
            </Select>
          )}
        </Box>
      ...
      {role === 'admin' && (
        <Box sx={{ display: 'flex', gap: 3, mt: 4 }}>
          <Box sx={{ flex: 1 }}>
            <Typography variant="subtitle1" gutterBottom>Assigned Gateways</Typography>
            <List sx={{ border: '1px solid #ddd', borderRadius: 2, maxHeight: 200, overflowY: 'auto' }}>
              {gateways.length > 0 ? (
                gateways.map((gateway) => (
                  <ListItem key={gateway.G_id} button>
                    <ListItemText primary={gateway.gateway_name} secondary={gateway.mac_address} />
                  </ListItem>
                ))
              ) : (
                <ListItem><ListItemText primary="No Assigned Gateways" /></ListItem>
              )}
            </List>
          </Box>
